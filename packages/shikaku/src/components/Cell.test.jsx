...
describe('Cell Component', () => {
  it('renders correctly with no clue', () => {
    render(<Cell x={0} y={0} clue={undefined} isCovered={false} onPointerDown={() => {}} onPointerEnter={() => {}} />);
    expect(screen.queryByText(/[0-9]/)).not.toBeInTheDocument();
  });

  it('renders a clue when provided', () => {
    render(<Cell x={0} y={0} clue={4} isCovered={false} onPointerDown={() => {}} onPointerEnter={() => {}} />);
    expect(screen.getByText('4')).toBeInTheDocument();
    expect(screen.getByText('4')).toHaveClass('font-bold');
  });

  it('opacity is reduced when covered', () => {
    render(<Cell x={0} y={0} clue={4} isCovered={true} onPointerDown={() => {}} onPointerEnter={() => {}} />);
