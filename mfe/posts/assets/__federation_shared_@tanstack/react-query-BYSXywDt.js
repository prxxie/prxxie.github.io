import { importShared } from '../__federation_fn_import-C1MTFLth.js';
...
var QueryClientContext = React$9.createContext(
  void 0
);
var useQueryClient = (queryClient) => {
  const client = React$9.useContext(QueryClientContext);
  if (queryClient) {
    return queryClient;
  }
  if (!client) {
...
var IsRestoringContext = React$8.createContext(false);
var useIsRestoring = () => React$8.useContext(IsRestoringContext);
...
var QueryErrorResetBoundaryContext = React$7.createContext(createValue());
var useQueryErrorResetBoundary = () => React$7.useContext(QueryErrorResetBoundaryContext);
var QueryErrorResetBoundary = ({
  children
}) => {
  const [value] = React$7.useState(() => createValue());
...
function useQueries({
  ...
  const defaultedQueries = React$5.useMemo(
    () => queries.map((opts) => {
...
function useBaseQuery(options, Observer, queryClient) {
  ...
  const [observer] = React$4.useState(
    () => new Observer(
      client,
      defaultedOptions
    )
...
var HydrationBoundary = ({
  ...
  const client = useQueryClient(queryClient);
  const optionsRef = React$3.useRef(options);
  React$3.useEffect(() => {
    optionsRef.current = options;
  });
  const hydrationQueue = React$3.useMemo(() => {
    if (state) {
...
function useMutationState(options = {}, queryClient) {
  ...
  const result = React$1.useRef(null);
