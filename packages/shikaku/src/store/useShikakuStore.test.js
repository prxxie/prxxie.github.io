...
const levelsListMock = [mockPuzzle];
...
describe('Shikaku Game State Store', () => {
  ...
  it('should handle region commits correctly and update saveProgress when won', () => {
    ...
    // Commit first 2x1 region for top clue {x:0, y:0, value:2}
    ...
    // Commit second 2x1 region for bottom clue {x:0, y:1, value:2}
    useShikakuStore.setState({ dragStart: { x: 0, y: 1 }, dragEnd: { x: 1, y: 1 } });
    ...
    expect(state.starsAchieved).toBe(3); // elapsedTime is 0, which <= targets.threeStars (10)
    
    // Check saved progress
    expect(state.completedLevels['test-1']).toEqual({ stars: 3, bestTime: 0 });
    const stored = JSON.parse(localStorage.getItem('cozy_os_shikaku_save'));
  ...
  it('should fail to commit invalid region', () => {
    ...
    // 1x1 region containing clue value 2 -> invalid area
    useShikakuStore.setState({ dragStart: { x: 0, y: 0 }, dragEnd: { x: 0, y: 0 } });
  ...
  it('should support removing a region at coordinate', () => {
    ...
    // Commit region (0,0) to (1,0)
    useShikakuStore.setState({ dragStart: { x: 0, y: 0 }, dragEnd: { x: 1, y: 0 } });
    useShikakuStore.getState().commitDrag();

    expect(useShikakuStore.getState().regions.length).toBe(1);

    // Try removing at empty coordinate (0,1)
    useShikakuStore.getState().removeRegionAt(0, 1);
    expect(useShikakuStore.getState().regions.length).toBe(1);

    // Remove at (1,0) - should succeed
    useShikakuStore.getState().removeRegionAt(1, 0);
    expect(useShikakuStore.getState().regions.length).toBe(0);
    expect(useShikakuStore.getState().history.length).toBe(2); // loadLevel(0) -> commit(1) -> remove(2) -> hist gets previous states
  });
  ...
  it('should load save state from localStorage', () => {
    localStorage.setItem('cozy_os_shikaku_save', JSON.stringify({
      completed: { 'test-1': { stars: 2, bestTime: 18 } }
    }));

    useShikakuStore.getState().loadSave();
    expect(useShikakuStore.getState().completedLevels['test-1']).toEqual({ stars: 2, bestTime: 18 });
  });
